...
## Post-merge follow-ups (2026-07-07)

The maintainer ran the interactive window (the one differential leg the automated flow could not exercise — no display), which surfaced three follow-ups, each fixed on its own branch and `--no-ff` merged:

1. **Controls (`3a315fe`).** Rebound to the maintainer's scheme: the arrow keys are the movement pad (`UP`/`DOWN` = forward/backward, `LEFT`/`RIGHT` = strafe, all facing-relative), `Q`/`E` = turn left/right.
2. **macOS window run (`ab9e17c`).** `exec:java` runs the main class on a Maven worker thread, which fails GLFW's mandatory "first thread in the process" check. A macOS-activated `run-macos` profile now forks a fresh JVM with `-XstartOnFirstThread` via `exec:exec`, running the launcher on that process's own main thread. Run: `mvn -pl beholder-render-libgdx compile exec:exec`.
3. **Collision bug — the design's "walls are symmetric" assumption was WRONG on real data (`7b53316`).** The interactive window showed the party walking *into* solid wall blocks. Real EOB1 stores walls **asymmetrically**: an open floor cell carries `OPEN` on the edge toward a neighbouring all-`SOLID` wall block, so checking only the party cell's own forward edge (the "current-cell edge is authoritative; symmetric for M3" claim in the Movement model + the outcome above) let the party step into the wall. **Corrected model:** a move is blocked if **either** the party cell's travel-direction edge **or** the target cell's facing-back edge is `SOLID` (a wall on either side of the crossed boundary). A regression test (asymmetric wall block: party edge `OPEN`, target edge `SOLID` → blocked) was RED before the fix; the existing symmetric-edge tests stay green. A scratch movement-probe over the real `LEVEL1.MAZ` confirmed the party now stops at wall blocks. The interactive start also moved from the M2 reference cell (10,12) — a 3-cell dead-end pocket — to **(7,21) facing EAST**, a ~9-cell open corridor, so movement is immediately visible.

**Lesson.** The "authoritative collision edge" was flagged in the design as the one thing to pin against kyra on real data; it was under-verified — the synthetic unit tests used *symmetric* geometry, and the real-data IT's structural invariants were satisfied by the buggy behaviour (walking into a wall block still stays on-grid and settles). **The human eyeball caught it**, which is exactly the leg §8 reserves for the human when no pixel-exact reference exists. Future collision-sensitive work should probe the real maze's asymmetry directly rather than assume symmetry.
